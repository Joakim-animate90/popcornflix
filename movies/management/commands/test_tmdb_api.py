        ...
        try:
            ...
            if tmdb_service.test_connection():
                ...
                if popular and popular.get('results'):
                    ...
                    self.stdout.write(
                        self.style.WARNING('⚠️  Popular movies endpoint returned no results')
                    )
                ...
                if search_results and search_results.get('results'):
                    self.stdout.write(
                        self.style.SUCCESS(
                            f'✅ Search: Found {len(search_results["results"])} results for "batman"'
                        )
                    )
                else:
                    self.stdout.write(
                ...
                self.stdout.write(
                    self.style.ERROR('❌ TMDb API connection failed!')
                )
                self.stdout.write('Please check your TMDB_BEARER_TOKEN or TMDB_API_KEY in .env file')
                
        except ValueError as e:
            self.stdout.write(
                self.style.ERROR(f'❌ Configuration error: {str(e)}')
            )
            self.stdout.write('Make sure TMDB_BEARER_TOKEN or TMDB_API_KEY is set in your .env file')
